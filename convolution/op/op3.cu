...
//     if (w < W_out && h < H_out && m < M && c < C) {
//         for (int b = 0; b < B; b++) {
//             for (int p = 0; p < K; p++) {
//                 for (int q = 0; q < K; q++) {
//                     int h_in = h * S + p;
//                     int w_in = w * S + q;
//                     float val = input[b * (C * H * W) + c * (H * W) + h_in * W + w_in] * 
//                                 mask[m * (C * K * K) + c * (K * K) + p * K + q];
//                     atomicAdd(&output[b * (M * H_out * W_out) + m * (H_out * W_out) + h * W_out + w], val);
//                 }
//             }
//         }
//     }
...
//     const float *device_mask, const int B, const int M, 
...
//     dim3 gridDim(gridW, gridH, gridZ); 
