...
__global__ void spmvJDSKernel(float *out, int *matColStart, int *matCols,
  ...
  if (tid < dim){
    float dot = 0;
    for (int sec = 0; sec < matRows[tid]; ++sec){
      int j = matColStart[sec] + tid;
      dot += matData[j] * vec[matCols[j]];
    }
    out[matRowPerm[tid]] = dot;
  }
}
...
static void spmvJDS(float *out, int *matColStart, int *matCols,
  ...
  spmvJDSKernel<<<gridSize, blockSize>>>(out, matColStart, matCols, matRowPerm, matRows, matData, vec, dim);
}
